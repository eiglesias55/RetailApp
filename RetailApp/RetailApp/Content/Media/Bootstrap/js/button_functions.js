$(document).ready(function(){
    ...
    $('span.pull-right a').click(function () {    
        if ($('#loginModal div.modal-dialog div.modal-footer span.pull-right a').text() === 'No deseas loguear con facebook?') {
            $('#loginModal div.modal-dialog div.modal-body').html("<div class='input-group input-group-lg'><span class='input-group-addon' id='sizing-addon1'>@</span><input type='text' class='form-control' placeholder='E-mail' aria-describedby='sizing-addon1'></div><button type='button' class='pull-right'>Enviar</button>");
